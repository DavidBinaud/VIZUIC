    		...
    		if ($tab_q != NULL) {
    			...
    			foreach ($tab_q as $q){
				    ...
				    if($q->get("typeChamp") == "nombre"){
				    	...
				    	echo "<p>
				    			<input placeholder = 'Max : {$q->get('valeurMaxChamp')}' type='" . $type . "' name='{$q->get('idChamp')}' id='{$q->get('idChamp')}' min='0' max='" . $q->get('valeurMaxChamp') . "' step='0.01' value='{$q->get('valeurChamp')}' required/>
			    			</p>";
